...
public class TowerUpgrade : MonoBehaviour
{
    ...
    public void TowerColorUpgrade(int colorindex)
    {
        ...
        if (GameManager.gmInstance.gmGold >= tuTowerColorUpgradeGold) // 업그레이드에 필요한 골드량 만큼 있는지 체크후 업그레이드 진행.
        {
            ...
            tuTowerUpgradeUI[colorindex].transform.DOPunchScale(new Vector3(0.1f, 0.1f, 0.1f), 0.5f, 1, 0.1f).SetEase(Ease.OutQuad).OnComplete(() =>
        {
        ...
        else
        {
            ...
            tuTowerUpgradeUI[colorindex].GetComponent<Button>().interactable = false;
            tuTowerUpgradeUI[colorindex].transform.DOShakePosition(1, new Vector3(10f, 1f, 10f), 5, 20, false, true).SetEase(Ease.OutQuad).OnComplete(() =>
        {
